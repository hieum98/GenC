...
class GradCache:
    ...
    def forward_backward(
        ...
        if isinstance(model, _FabricModule):  
            assert self.fabric is not None, "Fabric module requires a fabric instance to run forward-backward"
            no_syn_flags = [True for _ in range(len(model_inputs) - 1)] + [False]
            
            for x, state, gradient, flag in zip(model_inputs, random_states, cached_gradients, no_syn_flags):
                with self.fabric.no_backward_sync(model, enabled=flag):
                    ...
                    if self.fp16: 
                        ...
                        self.fabric.backward(surrogate)  
        elif isinstance(model, (nn.parallel.DistributedDataParallel, FullyShardedDataParallel)):
