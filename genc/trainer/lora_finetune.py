...
def fit(
    ...
    # Sanity check
    validate(fabric, model, tokenizer, val_dataloader, dataclasses.replace(validation_args, max_iters=5))
    ...
    dpo_running_loss = RunningMean(window=training_args.gradient_accumulation_iters(devices), sync_on_compute=False).to(
        fabric.device
    )
    kl_running_loss = RunningMean(window=training_args.gradient_accumulation_iters(devices), sync_on_compute=False).to(fabric.device)
    ...
    if training_args.use_gc:
        ...
        if fabric._precision.precision in ['16-true', '16-mixed', '16']:
        ...
        fabric.print("Gradcache initialized")
    while step_count < max_steps and train_iterator.epoch < training_args.num_train_epochs:
        iter_num += 1
        ...
        if training_args.use_gc:
            ...
            world_size = fabric.world_size if no_sync_except_last else 1
        ...
        for emb_input_chunk, gen_input_chunk in zip(split_input(emb_model_inputs, chunksize), split_input(gen_model_inputs, chunksize)):
            inner_iter_num += 1
            is_accumulating = iter_num % training_args.gradient_accumulation_iters(devices) != 0
            with fabric.no_backward_sync(model, enabled=is_accumulating):
                ...
                fabric.backward(loss/training_args.gradient_accumulation_iters(devices))

            kl_running_loss.update(loss_kl.detach())
            dpo_running_loss.update(dpo_losses.detach())

        # Do the optimizer step here as we do gradient accumulation in the inner loop
        optimizer.step()
        optimizer.zero_grad()
        scheduler.step()
        step_count += 1 
        if iter_num % training_args.log_interval == 0:
            ...
            throughput.update(time=t1 - total_t0, batches=iter_num, samples=iter_num * training_args.batch_size(devices))
        ...
        if step_count % validation_args.interval == 0:
            t0 = time.perf_counter()
            metrics = validate(fabric, model, tokenizer, val_dataloader, validation_args)
            val_metric = torch.tensor(metrics['R_at_1'])
            t1 = time.perf_counter() - t0
            fabric.print(f"iter {iter_num}: val loss {val_metric.item():.4f}, val time: {t1 * 1000:.2f} ms")
            fabric.log_dict(metrics, step=iter_num)
            fabric.barrier()
        
        if training_args.save_interval is not None and not is_accumulating and step_count % training_args.save_interval == 0:
            optim_checkpoint_path = checkpoint_dir / f"step_{step_count}" / "optim.ckpt"
            ...
            if fabric.global_rank == 0:
                model_save_dir = checkpoint_dir / f"step_{step_count}" / "model"
